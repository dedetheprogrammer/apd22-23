...
public:
    ...
    void export_clauses(std::string output, bool verbose_time) {
        ...
        int N_CLAUSES = 0;
        std::string literal;
        std::vector<std::string> r1(N_2, ""), r2(N_2, ""), r3(N_2, ""), r4(N_2, ""), r5;
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                ...
                for (int d = 0; d < N; d++) {
                    ...
                    // Restriccion 1:
                    r1[offset_nd] += literal;
                    for (int d_i = d+1; d_i < N; d_i++) {
                        r1.push_back("-" + literal + "-"
        ...
        N_CLAUSES += r1.size() + r2.size() + r3.size() + r4.size() + r5.size();
...
int main(int argc, char *argv[]) {
    ...
    std::string usage =
        "usage: ./practica2 -input <input_file> -output <output_file> -outsol <sol_file> [-sat <results> <n>] [-vet]\n";
