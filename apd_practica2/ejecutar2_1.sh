...
g++ main_1.cpp -std=c++11 -o practica2_1
if [ $? -ne 0 ]; then 
...
for i in ${!boards[@]}; do
    ...
    ./practica2_1 -input ${sb_path}"/"${b}".txt" -output ${sc_path}"/"${b}".cnf" -outsol $pass -vet
    size=$?
    ${ms_path} ${sc_path}"/"${b}".cnf" > ${ss_path}"/"${b}".sat" 2> ${ss_path}"/"${b}".res"
    ./practica2_1 -input ${ss_path}"/"${b}".sat" -output ${sst_path}"/"${b}".sat.sol" -sat ${ss_path}"/"${b}".res" $size -vet
done
